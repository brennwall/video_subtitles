...
audio = video.audio
subtitles = ffmpeg.input('clip.srt').filter("subtitles", 'clip.srt')

ffmpeg.concat(video.filter("scale", size="1280x720").filter("concat", n=2, v=1, a=0), subtitles, audio, v=0, a=1).output('out.mp4').run()
